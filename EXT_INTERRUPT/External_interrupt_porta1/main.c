...
void peripheral_bus()
{
	*RCC_AHB1ENR = *RCC_AHB1ENR | (1 << 0); //I/O PORT
	*RCC_APB2ENR = *RCC_APB2ENR | (1 << 14); // SET SYSCONFIG ENABLE
}

void gpio_moder()
{
	*GPIOA_MODER = (*GPIOA_MODER & ~(0X3 << 12)) | (1 << 12); // OUTPUT MODE
...
void exti_config()
{
	...
	*SYSCFG_EXTICR1 = (*SYSCFG_EXTICR1 & ~(0XF << 1)) | (0 << 0); // PA1->EXTI1
	...
	*NVIC_ISER0  |= (1 << 7); // External interrupt Exti1
}
