...
void peripheral_bus()
{
	*RCC_AHB1ENR = *RCC_AHB1ENR | (1 << 0); //I/O PORT ENABLE
...
void gpio_moder()
{
	...
	*GPIOA_PUPDR = (*GPIOA_PUPDR  & ~(0X3 << 0)) | (2 << 0); // PULL_DOWN
}
...
void exti_config()
{
	...
	*SYSCFG_EXTICR1 = (*SYSCFG_EXTICR1 & ~(0XF << 0)) | (0 << 0); // PAO->EXTI0
